...
def gen_tag_pages(tpl: str=None) -> None:
    """
    ...
    def _write_tag_page(name: str, links: list) -> None:
        result = []
        tag_fpath = ROOT / TAGS_DIR / f'{name}.md'
    ...
    tags = iterate_all_pages_tags(ROOT)
    with open(ROOT / TAGS_DIR / 'index.md', 'w', encoding='utf8') as tag_index_writer:
        ...
        for name in sorted(tags, key=lambda k: len(tags[k]), reverse=True):
            ...
            tag_index_writer.write('\n'.join(result))
            logging.info(f'{tag_index_writer} is updated successfully')
        ...
        for sub_fp in fp.iterdir():
            if sub_fp.is_file() and not sub_fp.name.startswith('index.md'):
                ...
                if sub_fp.suffix == '.html':
                    ...
                    link = sub_fp.name
                    md_result[date] = (title, link)
                    #writer.write(f'1. [{title}]({sub_fp.name})\n')
                # ignore other file types    

            # 目录递归
            if sub_fp.is_dir() and not sub_fp.name.startswith('_'):
                #writer.write(f'1. [{sub_fp.name}]({sub_fp.name}/index.md)\n')
                dir_result[sub_fp.name] = f'{sub_fp.name}/index.md'
                logging.debug(f'find dir {sub_fp.name}')
                index_me(sub_fp)
        for name in sorted(dir_result):
            writer.write(f'1. [{name}]({dir_result[name]})/\n')
        
        for date in sorted(md_result, reverse=True):
            writer.write(f'1. {date}, [{md_result[date][0]}]({md_result[date][1]})\n')
    return None
